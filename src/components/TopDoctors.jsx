...
const TopDoctors = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-4 my-8 md:my-16 text-gray-900 px-4 md:mx-10">
      ...
      <div className="w-full md:w-[80%] grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 pt-5 gap-y-4 sm:gap-y-6">
        {Array.isArray(doctors) &&
          doctors.slice(0, 8).map((item, index) => (
            ...
            >
              <img
                className="w-full object-contain h-32 sm:h-48"
                src={item.image}
                alt={item.name}
              />
