...
const Doctors = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        // First try to get doctors from the API
        const response = await axios.get(
          "https://backend-z1qz.onrender.com/api/doctors"
        );

        if (response.data && Array.isArray(response.data)) {
          // Merge API data with local image data
          const doctorsWithImages = response.data.map((doctor) => {
            const localDoctor = localDoctors.find((d) => d._id === doctor._id);
            return {
              ...doctor,
              image: localDoctor?.image || doctor.image,
            };
          });
          setDoctors(doctorsWithImages);
          const uniqueSpecialties = [
            ...new Set(doctorsWithImages.map((doctor) => doctor.speciality)),
          ];
          setSpecialties(uniqueSpecialties);
        } else {
          // If API response is not in expected format, use local data
          setDoctors(localDoctors);
          const uniqueSpecialties = [
            ...new Set(localDoctors.map((doctor) => doctor.speciality)),
          ];
          setSpecialties(uniqueSpecialties);
        }
      } catch (err) {
